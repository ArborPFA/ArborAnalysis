    ...
    Processor("PfoAnalysis")
{
    ...
    registerProcessorParameter( "HadronicEnergyResolution",
                                "Set the hadronic energy resolution",
...
void PfoAnalysis::init()
{
    ...
    m_tree->Branch("nPfosClusterless", &m_nPfosClusterless, "nPfosClusterless/I");
    ...
    m_tree->Branch("pfoChiTrack", &m_pfoChiTrack);
    ...
    m_hpfoEnergyNoCluster = new TH1F("fPFA_nocluster", "Energy of cluster-less pfos", 10000, 0., 5000.);
    m_hpfoEnergyNoCluster->SetDirectory(m_pTFile);
}
...
void PfoAnalysis::PerformPfoAnalysis()
{
    ...
    for (ParticleVector::const_iterator iter = m_pfoVector.begin(), iterEnd = m_pfoVector.end(); iter != iterEnd; ++iter)
    {
        ...
        if (!pPfo->getTracks().empty())
        {
            ...
            if( pPfo->getClusters().empty() )
            {
            	++m_nPfosClusterless;
            	m_pfoEnergyClusterless += pPfo->getEnergy();
            	m_hpfoEnergyNoCluster->Fill( pPfo->getEnergy() );
            }
            ...
            const float pz((*iter)->getMomentum()[2]);
            const float trackEnergySyum( std::sqrt( px*px + py*py + pz*pz ) );
